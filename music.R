...
data = fread("music/data.csv")
finaldata = fread("music/finaldata.csv")
...
variable = function(x){
  ...
  trainvar5 = merge(trainvar4, varmean, by = "variable", all.x = T)       #all.x  오류확인
  trainvar5[is.na(V1), V1 := 0.5]                                         #all.x  오류확인
